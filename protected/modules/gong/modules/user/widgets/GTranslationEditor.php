...
class GTranslationEditor extends GTag {
    ...
    public function init() {
        if($this->model == null) {
            ...
            $this->model = GTranslation::model()->find('`message` = :message', array(':message'=>$this->message));
        }
        else {
            $this->text .= BsHtml::labelBs($this->model->message);
        }
        $this->text .=
            $this->widget('GInlineEditable',
                    array_merge(
                            array('htmlOptions'=>$this->htmlOptions),
                            array(
                                'htmlOptions'=>array('id'=>$this->id.'-editable'),
                                'pk'=>$this->model->id,
