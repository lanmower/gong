...
class GFieldsController extends GController
{
	const PAGE_SIZE=10;
	...
	public function actionView()
	{
		$this->render('view',array(
			'model'=>$this->loadModel('GProfileField'),
		));
	}

	public function actionCreate() {
		$model = new GProfileField;

		// add to group?
		if(isset($_GET['in_group']))
			$model->field_group_id=$_GET['in_group'];

		if(isset($_POST['GProfileField'])) {
			$model->attributes = $_POST['GProfileField'];
			...
			if($model->validate()) {
				$sql = 'ALTER TABLE '.GProfile::model()->tableName().' ADD `'.$model->varname.'` ';
				$sql .= $field_type;
	...
	public function actionDelete()
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			$model = $this->loadModel('GProfileField');
			$sql = 'ALTER TABLE '.GProfile::model()->tableName().' DROP `'.$model->varname.'`';
	...
	{
		$dataProvider=new CActiveDataProvider('GProfileField', array(
			'pagination'=>array(
