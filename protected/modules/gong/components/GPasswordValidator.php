	...
	protected function validateAttribute($object, $attribute) {
		...
		if ($this->minLen > 0) {
			if (strlen ( $value ) < $this->minLen) {
				$message = $this->message !== null ? $this->message : Yii::t ( 'Gong.GPasswordValidator', '{attribute} is too short (min. {num} characters).', array (
						'{num}' => $this->minLen 
				) );
		...
		if ($this->maxLen > 0) {
			if (strlen ( $value ) > $this->maxLen) {
				$message = $this->message !== null ? $this->message : Yii::t ( 'Gong.GPasswordValidator', '{attribute} is too long (max. {num} characters).', array (
						'{num}' => $this->maxLen 
				) );
		...
		if ($this->minLowerCase > 0) {
			if (preg_match_all ( '/[a-z]/', $value, $matches ) < $this->minLowerCase) {
				$message = $this->message !== null ? $this->message : Yii::t ( 'Gong.GPasswordValidator', '{attribute} must include at least {num} lower case letters.', array (
		...
		if ($this->minUpperCase > 0) {
			if (preg_match_all ( '/[A-Z]/', $value, $matches ) < $this->minUpperCase) {
				$message = $this->message !== null ? $this->message : Yii::t ( 'Gong.GPasswordValidator', '{attribute} must include at least {num} upper case letters.', array (
		...
		if ($this->minDigits > 0) {
			if (preg_match_all ( '/[0-9]/', $value, $matches ) < $this->minDigits) {
				$message = $this->message !== null ? $this->message : Yii::t ( 'Gong.GPasswordValidator', '{attribute} must include at least {num} digits.', array (
						'{num}' => $this->minDigits 
				) );
				$this->addError ( $object, $attribute, $message );
			}
		...
		if ($this->minSym > 0) {
			if (preg_match_all ( '/\W/', $value, $matches ) < $this->minSym) {
				$message = $this->message !== null ? $this->message : Yii::t ( 'Gong.GPasswordValidator', '{attribute} must include at least {num} symbols.', array (
						'{num}' => $this->minSym 
				) );
		...
		if (! $this->allowWhiteSpace) {
			if (preg_match ( '/\s/', $value )) {
				$message = $this->message !== null ? $this->message : Yii::t ( 'Gong.GPasswordValidator', '{attribute} must not contain whitespace.', array (
						'{num}' => $this->minSym 
				) );
		...
		if ($this->maxRepetition > 0) {
			if (preg_match ( '/(.){1}\\1{' . $this->maxRepetition . ',}/', $value )) {
				$message = $this->message !== null ? $this->message : Yii::t ( 'Gong.GPasswordValidator', '{attribute} must not contain more than {num} sequentially repeated characters.', array (
						'{num}' => $this->maxRepetition + 1 
				) );
