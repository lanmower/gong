...
class GViewDom extends GField {
    ...
    public function run() {
        ...
        echo CHtml::link($this->copyText, 'javascript:', array('onclick' => "
            local = $(this);
        ...
        echo CHtml::tag('div', array('style' => 'display:none;position:absolute;', 'class' => $this->id . '-data'));
        echo CHtml::link($this->pasteText, 'javascript:', array('onclick' => 'data = $(this).parent().find(".data"); data.attr("value", window.prompt("Paste from clipboard: Ctrl+V, Enter\n")); if(data.attr("value")) $(this).parent().submit();'));
        echo CHtml::hiddenField('dom', '', array('class' => 'data'));
        echo CHtml::submitButton('', array('style' => 'visibility:hidden;'));
        echo CHtml::endForm();
        parent::run();
    }
