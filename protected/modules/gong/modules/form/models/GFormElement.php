...
class GFormElement extends GElement {
    ...
    public function getCreateRedirect() {
        return Yii::app()->controller->createUrl('/gong/form/admin/view/', array('id'=>$this->parent->hash));
    }
    ...
    public function rules() {
        return array(
            ...
            array('prefixContent, suffixContent', 'filter', 'filter' => array($obj = new CHtmlPurifier(), 'purify')),
            array('className, title, description, name, fields', 'safe'),
        );
    }
    ...
    public function getSql() {
        ...
        if (property_exists($this->className, 'model'))
            $fieldWidget->model = $this;
        if(method_exists($fieldWidget, 'beforeInit'))$fieldWidget->beforeInit();
