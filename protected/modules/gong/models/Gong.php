...
class Gong {
    ...
    public static function t($string, $args = null, $category = 'gong', $language = 'en') {
            ...
            if($data != false) {
                if(isset($data['translation'])) return sprintf($data['translation'], $args);
            }
     ...
     * @param type $string the class string to add to 
     * @param type $class the class to add to the string
     * @return string the modified class string
     */
    public static function addClassToString(&$string, $class) {
        if (!isset($string)) $string = $class;
        else
            $string .= ' ' . $class;
        return $string;
    }
    ...
    public static function requiredFieldNote() {
        return CHtml::tag('p', array('class' => 'note'), G::t(
