...
class GSubmissionController extends GController {
    public function accessRules() {
        ...
        self::addRule($rules, array('allow', // allow admin user to perform 'admin' and 'delete' actions
                'actions' => array('form', 'index','captcha'),
                'users' => array('@'),
            ));
        return $rules;
    }
	...
	if(!Yii::app()->user->can($form->view)) throw new CHttpException(403, 'User is not allowed to use this form');       
	$this->setVar('form', $form);
        if (isset($_POST['pk'])) {
            ...
            if (!$model->save()) {
                foreach($model->errors as $error) echo "<span class='alert-error'>{$error[0]}</span>";
	...
	if(!Yii::app()->user->can($form->view)) throw new CHttpException(403, 'User is not allowed to use this form');       
        ...
        if (isset($_POST['GSubmission'])) {
            ...
            if ($model->save())
                $this->render('updateDone', array('model' => $model));
        } else {
    ...
    public function actionIndex($id) {
        $form = GForm::model()->findByPk(PseudoCrypt::unhash($id));
 	if(!Yii::app()->user->can($form->view)) throw new CHttpException(403, 'User is not allowed to view this form');       
       if ($form === null)
            throw new CHttpException(404, 'The requested page does not exist.');
        $this->setVar('form', $form);
    ...
    public function actionDelete($id, $dataId) {
	    ...
	    if(!Yii::app()->user->can($form->view)) throw new CHttpException(403, 'User is not allowed to view this form');       
            ...
            $this->redirect($this->createUrl("index", array('id'=>$form->hash)));
        }
    }
	...
	if(!Yii::app()->user->can($form->view)) throw new CHttpException(403, 'User is not allowed to view this form');       
        ...
        $this->render('form', array('model' => $data));
    }
