<?php
Yii::app ()->clientScript->registerScript ( "formPageScript-$page->id", isset ( $_GET ['scroll'] ) ? '$("html, body").animate({scrollTop: $("#' . $_GET ['scroll'] . '").offset().top}, 250);' : '$("html, body").animate({scrollTop: "0px"}, 250);' );
...
echo $formWidget->errorSummary ( $data, "", "", $htmlOptions = array (
		'class' => 'errorSummary alert alert-danger' 
) );
echo $formWidget->errorSummary ( $regForm, "", "", $htmlOptions = array (
		'class' => 'errorSummary alert alert-danger' 
) );
echo $formWidget->errorSummary ( $profile, "", "", $htmlOptions = array (
		'class' => 'errorSummary alert alert-danger' 
) );
