...
class GTable extends GTag {
    ...
    public $fields = false;
    public $actions;
    public $tableOptions = array('class' => 'table');
    public $importJsonEditor;

    public function imports() {
        if ($this->dataTables) {
            $this->core[] = array('name' => 'jquery.ui');
            $this->core[] = array('name' => 'jquery');
            $datatablesUrl = Yii::app()->assetManager->publish('protected/vendors/datatables', false, -1, YII_DEBUG);
            if (YII_DEBUG)
                $this->js[] = array('url' => $datatablesUrl . '/media/js/jquery.dataTables.min.js', CClientScript::POS_BEGIN);
            else
                $this->js[] = array('url' => $datatablesUrl . '/media/js/jquery.dataTables.js', CClientScript::POS_BEGIN);
        ...
        foreach ($data as $rowkey => $row) {
            ...
            foreach ($columns as $col) {
                try {
                    ...
                    $errorOptions = array('class' => 'errorElement', 'data-errorlinktext' => "Error in GTable field {$col['name']}: {$e->getMessage()}");
                ...
                if (!isset($column['display']) || $column['display']) {
                    if (isset($column['editor'])) {
                        switch ($column['editor']) {
                            case 'textarea':
                                $updated[$column['id']] =
                                        $this->widget('GInlineEditable', array('pk' => PseudoCrypt::unhash(
                                            CHtml::value($row, $columns['id']['field'])),
                                    'htmlOptions' => array('id' => $this->id . $columns['id']['field'] . '-editor', 'class' => 'titleEditor'),
