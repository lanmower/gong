...
class GTabs extends GTag {
	...
	public function init() {
		if (! isset ( $this->tabsOptions ['htmlOptions'] ))
			$this->tabsOptions ['htmlOptions'] = array ();
		$tabs = array ();
		$setTab = true;
		foreach ( $this->items as $tab ) {
			if (isset ( $tab ['view'] ) && ! Yii::app ()->user->can ( $tab ['view'] ))
				continue;
			if (isset ( $tab ['type'] )) {
				if ($tab ['type'] == 'login') {
			...
			unset ( $tab ['view'] );
			if($tab['url'] == Yii::app()->request->requestUri) $tab['active'] = true;
		...
		if (! isset ( $this->tabsOptions ['id'] ))
			$this->tabsOptions ['id'] = $this->hash . '-tabs';
