...
class GUserChangePassword extends GFormModel 
{
	...
	public function rules() {
		...
		$passwordrule = array_merge(array(
					'password', 'GPasswordValidator'), $passwordRequirements);
		$rules[] = $passwordrule;
		$rules[] = array('currentPassword', 'safe');
		$rules[] = array('currentPassword', 'required', 'on' => 'user_request');
		$rules[] = array('password, verifyPassword', 'required');
		$rules[] = array('password', 'compare', 'compareAttribute' =>'verifyPassword', 'message' => G::t('Retyped password is incorrect'));
