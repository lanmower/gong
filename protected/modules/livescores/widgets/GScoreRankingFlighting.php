...
class GScoreRankingFlighting extends GTag {
    ...
    public function init() {
        $this->script = '
					...
					';
					if(isset($_GET['scroll'])) $this->script .= '$("html, body").animate({ scrollTop: $(document).height() }, 120000, "linear");
    setTimeout(function() {
            ...
            foreach ($players as $player) {
                if ($player->flighting == $flighting->id) {
                    ...
                    foreach ($scores as $score) {
                        if($score->playerId == $player->id) $nettTotal += $score->score;
                        if($score->playerId == $player->id && $score->score > 0) $grossTotal += $score->score;
                    }

                    $playerData['id'] = $player->id;
                    $playerData['grossScore'] = $grossTotal;
                    $playerData['nettScore'] = $nettTotal;
		    if($playerData['grossScore'] < 0) $playerData['grossScore'] = 0;
		    if($playerData['nettScore'] < 0) $playerData['nettScore'] = 0;

                    $playerData['name'] = $player->name;
                    foreach($flags as $flag) {
						if($flag->id == $player->country) $playerData['flag'] = $flag->flag;
					}
