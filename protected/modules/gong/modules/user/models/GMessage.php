...
class GMessage extends GActiveRecord {
        ...
        if (parent::beforeValidate()) {
            ...
            if ($to_user && isset($to_user->privacy)) {
                if (in_array($this->from_user->username, $to_user->privacy->getIgnoredUsers()))
                    $this->addError('toUserId', G::t('One of the recipients ({username}) has ignored you. Message will not be sent!', array('{username}' => $to_user->username)));
            }
            return true;
        }
        return false;
    ...
    public function beforeSave() {
        if ($this->isNewRecord) {
            ...
            if ($this->to_user->privacy && $this->to_user->privacy->message_new_message) {
                G::log(G::t(
                                'Message id {id} has been sent from user {fromUserId} to user {toUserId}', array(
                            '{id}' => $this->id,
    ...
    public static function write($to, $from, $subject, $body, $mail = true) {
        ...
        if (is_object($from))
            ...
            $message->fromUserId = $from;
                ...
                && $user = GUser::model()->find("username = '{$from}'"))
        ...
        if (is_object($to))
            ...
            $message->toUserId = $to;
                ...
                && $user = GUser::model()->find("username = '{$to}'"))
    ...
    public function rules() {
        return array(
            array('fromUserId, toUserId, subject', 'required'),
            array('fromUserId, draft, read, answered', 'numerical', 'integerOnly' => true),
    ...
    public function scopes() {
        $id = Yii::app()->user->id;
        return array(
            'all' => array('condition' => "toUserId = {$id} or fromUserId = {$id}"),
            'read' => array('condition' => "toUserId = {$id} and `read` = 1"),
            'unread' => array('condition' => "toUserId = {$id} and `read` = 0"),
            'sent' => array('condition' => "fromUserId = {$id}"),
            'received' => array('condition' => "toUserId = {$id}"),
            'answered' => array('condition' => "toUserId = {$id} and answered > 0"),
        );
    }
    ...
    public function relations() {
        return array(
            'from_user' => array(self::BELONGS_TO, 'GUser', 'fromUserId'),
            'to_user' => array(self::BELONGS_TO, 'GUser', 'toUserId'),
        );
    }
}
