...
class GProfileComment extends GActiveRecord{
	...
	{
		return array(
				array('userId, profile_id, comment, createtime', 'required'),
				array('userId, profile_id, createtime', 'numerical', 'integerOnly'=>true),
				array('id, userId, profile_id, comment, createtime', 'safe', 'on'=>'search'),
				);
	}
	...
	public function relations()
	{
		return array(
				...
				'profile' => array(self::BELONGS_TO, 'GProfile', 'profile_id'),
				);
	}
	...
	public function afterSave() {
		...
		if($user = $this->profile->user) {
			if($user->privacy 
				...
				GMessage::write($user, $this->userId,
						G::t('New profile comment from {username}', array(
