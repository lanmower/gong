...
class GForm extends GActiveRecord {
   ...
   public function getGridColumns() {
        return array (
            ...
            array('name'=>'table','class'=>'GEditableColumn'),
            array('header'=>'actions','class'=>'GButtonColumn', 'template'=>'{self}{custom}{delete}')
            );
    }
    public function behaviors() {
        return array(
            'DeleteChildren' => array(
                ...
                'childAttributes' => array('children')
            ),
            'Timestamp' => array(
            ...
            if (method_exists($field->className, 'getRules')) {
                ...
                foreach ($fieldRules as $fieldRule) {
                    if(is_array($fieldRule)) {
                        $rules[] = array_merge(array($field->name), $fieldRule);
                    } else
                    $rules[] = array($field->name, $fieldRule);
                }
    ...
    public function getModelGridColumns() {
        ...
        $gridColumns[] = array('header'=>'actions','class'=>'GSubmissionButtonColumn', 'template'=>'{update}{{delete}');
        return $gridColumns;
    }
