...
class GProfile extends GActiveRecord
{
	...
	public function getPublicFields() {
		...
		if($privacy = GUser::model()
			...
			foreach(GProfileField::model()->cache(3600)->findAll() as $field) {
				if(
						(($i & $privacy) 
						 && $field->visible != GProfileField::VISIBLE_HIDDEN) 
						|| $field->visible == GProfileField::VISIBLE_PUBLIC)
					$fields[] = $field;
				$i*=2;
			}
		}
	...
	public function relations()
	{
		$relations = array(
				...
				'comments' => array(self::HAS_MANY, 'GProfileComment', 'profile_id'),
				);

		$fields = Yii::app()->db->cache(3600)->createCommand(
				"select * from ".GProfileField::model()->tableName()." where field_type = 'DROPDOWNLIST'")->queryAll();
