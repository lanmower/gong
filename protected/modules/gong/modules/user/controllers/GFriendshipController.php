...
class GFriendshipController extends GModelController {
    ...
    public function accessRules() {
        return array(
            array('allow',
                'actions' => array('create', 'list', 'listAll', 'update', 'inlineUpdate',  /* 'dataUpdate', */ 'view'),
                'expression' => array('GFriendshipController', 'canUpdate'),
            ),
            array('allow',
                'actions' => array('delete', 'invite', 'approve', 'listPending', 'listApproved'),
                'users' => array('@'),
            ),
            array('deny',
                'users' => array('*'),
    ...
    public function loadModel($id) {
        ...
        $model = $className::model()->findByPk(PseudoCrypt::unhash($id), 'inviterId = :uid or friendId = :uid', array(':uid' => Yii::app()->user->id));
        if ($model == null)
    ...
    public function actionApprove($id = null) {
        if(isset($id)) {
            ...
            if (isset($friendship)) {
                $friendship->status = 2;
                if($friendship->save(true, array('status'))) {
        ...
        $this->render('listPending', array('friendships' => $model));
    }
    
    public function actionListApproved($id = null) {
            $model = Yii::app()->user->data->getApprovedFriendships();
            $this->render('listApproved', array('friendships' => $model));
    }
    ...
    public function actionInvite($friendId = null) {
        if ($friendId != null) {
            ...
            if (isset($friend)) {
                ...
                if($model->save()) G::setFlash('alert-info', 'Link requested.');
            } else {
                $model = new GFriendship();
        ...
        if(Yii::app()->user->can('Producer')) $reporters = GUser::model()->withRole('Reporter')->withoutFriends(Yii::app()->user->id)->findAll();
        GUser::model()->resetScope();
        if(Yii::app()->user->can('InviteAll')) $all = GUser::model()->withoutFriends(Yii::app()->user->id)->findAll();
        $this->render('invite', array('model' => $model, 'all'=>$all, 'reporters'=>$reporters));
    }
}
?>
