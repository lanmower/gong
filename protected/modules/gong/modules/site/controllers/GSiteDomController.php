...
class GSiteDomController extends GModelController {
    ...
    public function accessRules() {
        ...
        self::addRule($rules, array('allow',
            'actions' => array('import'),
            'users' => array('*'),
        ));
        self::addRule($rules, array('allow',
            'actions' => array('batchExport', 'batchImport'),
            'users' => array('*'),
        ));
        return $rules;
    }
    ...
    public function actionImport($type, $id) {
        ...
        if (isset($_POST['dom'])) {
            ...
            $this->redirect($this->createUrl($type . '/view', array('id' => $id)));
        }
    }
    ...
    public function actionBatchImport($type, $id = null, $module = null, $controller = null, $name = null) {
        ...
        if (isset($_POST['dom'])) {
            ...
            foreach ($children as $v) {
                ...
                $model = $modelClassName::model()->find('module = :module AND controller = :controller AND name = :name', array('module'=>$module,':controller' => $controller,':name' => $name));
                if (!$model) {
                    if(isset($id)) $model == $modelClassName::model()->findByPk(PseudoCrypt::unhash ($id));
                    else $model = new $modelClassName();
                } else {
                    unset($v['name']);
                }
                unset($v['id']);
    ...
    public function actionBatchExport($type = 'view', $id = null, $module = null, $controller = null, $name = null) {
        $modelClassName = $this->modelClassname = 'GSite' . ucfirst($type);
        if(isset($id)) {
            $output = array($this->getAttributesAndChildren($modelClassName::model()->findByPk(PseudoCrypt::unhash($id))));
        } else
        if (isset($module) && isset($controller) && isset($name)) {
            $output = array($this->getAttributesAndChildren($modelClassName::select($module, $controller, $name)));
        } else
        if (isset($module) && isset($controller)) {
            $views = $modelClassName::model()->findAll('module = :module AND controller = :controller', array(':module' => $module, ':controller' => $controller));
            $output = array();
        ...
        if (isset($module)) {
            $views = $modelClassName::model()->findAll('module = :module', array(':module' => $module));
            $output = array();
            foreach ($views as $view) {
                $output[] = $this->getAttributesAndChildren($view);
            }
