...
class GMembership extends GActiveRecord {
    ...
    public function rules() {
        return array(
            array('roleId, userId, payment_id, order_date', 'required'),
            array('end_date', 'default', 'setOnEmpty' => true, 'value' => null),
            array('roleId, userId, payment_id, order_date, end_date, payment_date, subscribed', 'numerical', 'integerOnly' => true),
            array('id, roleId, userId, payment_id, order_date, end_date, payment_date', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function relations() {
        return array(
            ...
            'role' => array(self::BELONGS_TO, 'GRole', 'roleId'),
            'payment' => array(self::BELONGS_TO, 'GPayment', 'payment_id'),
        );
    }
    ...
    public function sendPaymentConfirmation() {
        $return = GMessage::write($this->user, 1, G::t('Payment received'), strtr(
                                'The payment of order {id} has been received on {payment_date}', array(
    ...
    public static function syncMemberships() {
        if (!Yii::app()->user->isGuest) {
            foreach (GMembership::model()->findAll(
                    'userId = :uid and end_date != 0 and end_date < :date', array(
                ':uid' => Yii::app()->user->id,
