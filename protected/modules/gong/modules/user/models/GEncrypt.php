...
class GEncrypt {
	...
	public static function encrypt($string, $salt = null) {
		...
		return GEncrypt::pbkdf2 ( $string, $salt );
	}
	/**
	...
	public static function validate_password($password, $good_hash, $salt) {
		$enc_pwd = GEncrypt::encrypt ( $password, $salt );
		return GEncrypt::slow_equals ( $enc_pwd, $good_hash );
	}
	...
	private static function pbkdf2($password, $salt, $algorithm = 'sha256', $count = 1000, $key_length = 64, $raw_output = false) {
		...
		if ($raw_output)
			return substr ( implode ( $output ), 0, $key_length );
		else
			return bin2hex ( substr ( implode ( $output ), 0, $key_length ) );
	}
}
?>
