...
if($file->processed) {
    echo CHtml::tag('td', array(), CHtml::link('Play', "", array('class' => "nohijack playButton", 'onclick' => "$('.jp-volume, .jp-controls, .jp-progress, .jp-playlist').css({display:'inline-block'}); $.alm.player.add({".  implode("\n", $videoFile)."}, true); $.alm.player.play();")));
    echo CHtml::tag('td', array(), CHtml::link('Queue', "", array('class' => 'nohijack queueButton', 'onclick' => "$('.jp-volume, .jp-controls, .jp-progress, .jp-playlist').css({display:'inline-block'}); $.alm.player.add({".implode("\n", $videoFile)."});")));
} else echo CHtml::tag('td', array("colspan"=>2), CHtml::tag('span', array("class"=>"alert-info"), "Queued for processing."));
?>
