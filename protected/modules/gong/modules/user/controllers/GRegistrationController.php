...
class GRegistrationController extends GController {
	 ...
	 * The User will receive an email with an
			...
			if (isset ( $_POST ['GPasswordRecoveryForm'] )) {
				...
				if ($form->validate ()) {
					...
					if ($form->user instanceof GUser) {
						...
						$recovery_url = CHtml::link ( 'Click here', $this->createAbsoluteUrl ( "registration/recovery", array (
								'key' => $form->user->activationKey,
								'email' => $form->user->profile->email 
						) ) );
						
						G::log ( G::t ( '{username} successfully requested a new password in the password recovery form. A email with the password recovery url {recovery_url} has been sent to {email}', array (
								'{email}' => $form->user->profile->email,
								'{recovery_url}' => $recovery_url,
								'{username}' => $form->user->username 
						) ) );
						try {
							$body = $this->renderPartial ( "recoveryMail", array (
									'username' => $form->user->username,
									'recovery_url' => $recovery_url 
							) );
						} catch ( Exception $e ) {
						...
						G::log ( G::t ( 'A password has been requested, but no associated user was found in the database. Requested user/email is: {username}', array (
	...
	public function sendRegistrationEmail($user) {
		...
		$body = strtr ( 'Hello, {username}. Please activate your account with this url: {activation_url}', array (
				'{username}' => $user->username,
		...
		$mail = array (
				...
				'subject' => strtr ( 'Please activate your account for {site_name}', array (
						'{site_name}' => Yii::app ()->name 
				) ),
				'body' => $body 
		);
	 ...
	 * The Email and the Activation key send
