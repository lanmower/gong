    ...
    <?php
				foreach ( $profile->loadProfileFields () as $field ) {
					...
					if ($field->field_type == 'BOOLEAN') {
						...
						if ($field->items)
							echo CHtml::activeDropDownList ( $profile, $field->varname, unserialize ( $field->items ) );
					} else if ($field->field_type == "CHECKBOXLIST") {
						if ($field->items)
							echo CHtml::activeCheckBoxList ( $profile, $field->varname, unserialize ( $field->items ) );
					} else if ($field->field_type == "LISTBOX") {
						if ($field->items)
							echo CHtml::activeListBox ( $profile, $field->varname, unserialize ( $field->items ) );
