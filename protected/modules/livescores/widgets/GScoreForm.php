...
class GScoreForm extends GTag {
    ...
    public function init() {
        ...
        $this->js[] = array('url' => $tablecloth . "/js/jquery.tablecloth.js");

        $this->css[] = array('url' => $tablecloth . "/css/tablecloth.css");
    ...
    public function run() {
        ...
        if (isset($_POST['Course']) && is_numeric($_POST['Course'])) {
            ...
            $pars = GSubmission::forForm('Par')->findAllByAttributes(array('course' => $courseId), array('order' => 'hole'));
            $group = GSubmission::forForm('Group')->findByAttributes(array('course' => $courseId));
            $grossTotal = 0;
            $coursePar = 0;
            if (!$group) {
            ...
            $this->widget('GActiveDropDown', array(
                ...
                'listOptions' => array('onchange' => '$(this.form).trigger("submit");'),
                'name' => 'player',
                    )
            );
            ...
            if (isset($_POST['player']) && is_numeric($_POST['player'])) {
                ...
                $scores = GSubmission::forForm('Score')->findAll('created = :created AND playerId = :player AND course = :course', array('created' => date('Y-m-d'), ':player' => $player->id, ':course' => $courseId));
                ...
                echo CHtml::openTag('table', array('table'));
                echo CHtml::openTag('tr');
                echo CHtml::tag('th', array(), 'Out');
                echo CHtml::tag('th', array('colspan' => '9'));
                echo CHtml::openTag('tr');
                ...
                if ($player->modified != date('Y-m-d') || $player->lock == 'unlock') {
                    ...
                    echo CHtml::submitButton('Confirm', array('class' => 'btn btn-default', 'name' => 'confirm'));
                    echo CHtml::button('Clear form', array('class' => 'btn btn-default', 'onclick' => '$(this.form).append("<input type=\'hidden\' name=\'clear\' value=\'true\'/>");this.form.submit(); '));
                }
            }
        }
        parent::run();
