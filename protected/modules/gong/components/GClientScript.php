...
class GClientScript extends CClientScript {
    ...
    public function renderBodyEnd(&$output) {
        ...
        if (!Yii::app()->request->isAjaxRequest && !isset($_POST['isAjaxRequest']) && !isset($_GET['isAjaxRequest'])) {
            if(isset($this->scriptFiles[self::POS_END]))$this->called($this->scriptFiles[self::POS_END]);
    ...
    public function renderBodyBegin(&$output) {
        $html = '';
        if (!Yii::app()->request->isAjaxRequest && !isset($_POST['isAjaxRequest']) && !isset($_GET['isAjaxRequest'])) {
            if(isset($this->scriptFiles[self::POS_BEGIN])) $this->called($this->scriptFiles[self::POS_BEGIN]);
    ...
    private function called($scripts, $css = false) {
        if (isset($scripts)) {
            foreach ($scripts as $url=>$url2) {
                if($css) $this->_called[] = "\"$url2\"";
                else $this->_called[] = "\"$url\"";
            }
        }
    }
    ...
    public function renderHead(&$output) {
        ...
        if (!Yii::app()->request->isAjaxRequest && !isset($_POST['isAjaxRequest']) && !isset($_GET['isAjaxRequest'])) {
            ...
            if(isset($this->scriptFiles[self::POS_HEAD])) $this->called($this->scriptFiles[self::POS_HEAD]);
