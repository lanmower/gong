 ...
 * saving our Model to the Database. LetÂ´s assume the following Relation:
 *
 * Post has:
 ...
 * the Posts with the id 5, 6, 7 and 10 getÂ´s added up to our new Category.
 *
 ...
 * further in the future, so only one query getÂ´s executed for the MANY_MANY
 * Table.
 *
...
class CAdvancedArbehavior extends CActiveRecordBehavior {
	...
	protected function getRelations() {
		...
		foreach ( $this->owner->relations () as $key => $relation ) {
			if ($relation [0] == CActiveRecord::MANY_MANY && ! in_array ( $key, $this->ignoreRelations ) && $this->owner->hasRelated ( $key ) && $this->owner->$key != - 1) {
				$info = array ();
				$info ['key'] = $key;
