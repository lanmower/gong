...
class GUser extends GActiveRecord {
    ...
    public function getActiveRoles() {
        if(isset($this->_roles)) return $this->_roles;
        $roles = $this->roles;
    ...
    public function getPermissions() {
        if($this->_permissions == null) return $this->_permissions = $this->makePermissions();
    ...
    public function addToAccount($amount) {
        $account = GAccount::model()->find('userId = :uid', array(':uid'=>$this->id));
        if(!isset($account)) {
    ...
    public function getActiveMemberships() {
        if($this->_memberships == null) return $this->_memberships = $this->makeActiveMemeberships();
        else return $this->_memberships;
    }
    ...
    public function rules() {
        ...
        $passwordrule = array_merge(array('password', 'GPasswordValidator'), $passwordRequirements);
    ...
    public function relations() {
        if ($this->_relations == null) {
            ...
            $relations['roles'] = array(
                self::MANY_MANY, 'GRole',
                "{{user_role}}" . '(userId, roleId)');

            $relations['messages'] = array(
                self::HAS_MANY, 'GMessage', 'toUserId',
            ...
            $relations['sent_messages'] = array(
                self::HAS_MANY, 'GMessage', 'fromUserId');


            $relations['visits'] = array(
                self::HAS_MANY, 'GProfileVisit', 'visited_id');
            $relations['visited'] = array(
                self::HAS_MANY, 'GProfileVisit', 'visitor_id');
            $relations['profile'] = array(
                self::HAS_ONE, 'GProfile', 'userId');
            $relations['privacy'] = array(
                self::HAS_ONE, 'GPrivacySetting', 'userId');

            $relations['friendshipsRequested'] = array(
                self::HAS_MANY, 'GFriendship', 'inviterId',
                'on' => 'friendshipsRequested.status = 1', 'together'=>true);
            $relations['friendshipsAccepted'] = array(
                self::HAS_MANY, 'GFriendship', 'friendId',
                'on' => 'friendshipsAccepted.status = 2', 'together'=>true);
            
            $relations['friendsRequested'] = array(
                self::HAS_MANY, 'GUser', array('inviterId'=>'id'), 'through'=>'friendships', 'condition' => 'friendsRequested.status = 2');
            $relations['friendsAccepted'] = array(
                self::HAS_MANY, 'GUser', array('friendId'=>'id'), 'through'=>'friendships', 'condition' => 'friendsAccepted.status = 2');
            ...
            $relations['memberships'] = array(
                self::HAS_MANY, 'GMembership', 'userId');
    ...
    {
        $role = GRole::model()->find('title = :role', array(':role'=>$roleName));
        $this->getDbCriteria()->mergeWith(array(
            'condition'=>'memberships.roleId = :roleId',
            'with'=>array('memberships'),
    ...
    public function getMessageLink() {
        return CHtml::link(CHtml::tag('i', array('class'=>'glyphicon glyphicon-envelope')),'/gong/user/message/send/id/'.$this->hash);
    }
    
    public function getInviteLink() {
        return CHtml::link(CHtml::tag('i', array('class'=>'glyphicon glyphicon-link')),'/gong/user/friendship/invite/friendId/'.$this->hash);
    }

    public function isFriendOf($userId) {
        foreach ($this->getFriendships() as $friendship) {
            if ($friendship->inviterId == $this->id && $friendship->friendId == $userId ||
                    $friendship->friendId == $this->id && $friendship->inviterId == $userId)
                return true;
        }
    ...
    public function getFriendships() {
        return GFriendship::model()->findAll('inviterId = :uid or friendId = :uid', array(':uid' => $this->id));
    }
    
    public function getPendingInvitations() {
        return GFriendship::model()->findAll('friendId = :uid AND status = :status', array(':uid' => $this->id, ':status'=> GFriendship::STATUS_PENDING));
    }

    public function getPendingFriendships() {
        return GFriendship::model()->findAll('(inviterId = :uid or friendId = :uid) AND status = :status', array(':uid' => $this->id, ':status'=> GFriendship::STATUS_PENDING));
    }

    public function getApprovedFriendships() {
        return GFriendship::model()->findAll('(inviterId = :uid or friendId = :uid) AND status = :status', array(':uid' => $this->id, ':status'=> GFriendship::STATUS_APPROVED));
    }
            ...
            if ($user = $profile->user) {
                ...
                if ($user->activationKey == $key) {
                    ...
                    if ($user->save(false, array('activationKey', 'status'))) {
                        ...
                        GMessage::write($user, 1, G::t('Your activation succeeded'), strtr(
                                        'The activation of the account {username} succeeded. Please navigate to the login page to start using the site', array(
                                    '{username}' => $user->username,
                                    '{link_login}' =>
    ...
    public function withRole($role)
    {
        $this->getDbCriteria()->mergeWith(array(
            ...
            'with'=>array('memberships'=>array('with'=>'role'), 'roles'),
