...
echo CHtml::activeRadioButtonList ( $model, 'roleId', CHtml::listData ( GRole::model ()->findAll ( 'price != 0' ), 'id', 'description' ), array (
...
echo CHtml::activeRadioButtonList ( $model, 'payment_id', CHtml::listData ( GPayment::model ()->findAll (), 'id', 'title' ), array (
					'template' => '<div style="float:left;margin-right:5px;">{input}</div>{label}' 
			) );
			?>
			<div class="clear"></div>
