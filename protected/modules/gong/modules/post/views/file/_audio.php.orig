...
if($file->processed) {
    echo CHtml::tag('td', array(), CHtml::link('', "", array('class' => "nohijack playButton glyphicon glyphicon-play", 'onclick' => "$('.jp-volume, .jp-controls, .jp-progress, .jp-playlist').css({display:'inline-block'}); $.alm.player.add({".  implode("\n", $song).", playNow: true}); $.alm.player.play();")));
} else echo CHtml::tag('td', array("colspan"=>2), CHtml::tag('span', array('alert-info'), "Queued for processing."));
?>
